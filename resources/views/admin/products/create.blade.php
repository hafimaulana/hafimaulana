...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-8">
        <h1 class="text-4xl font-bold text-amber-400 mb-2 font-serif">Add New Product</h1>
        <p class="text-gray-400 text-lg">Create a new coffee product for your catalog</p>
    </div>
        ...
        <div class="bg-gray-800/50 border border-gray-700 rounded-2xl p-8">
            <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="category" id="category" required
                                ...
                                <option value="Decaf" {{ old('category') == 'Decaf' ? 'selected' : '' }}>Decaf</option>
                            </select>
                            @error('category')
                            <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="roast_level" id="roast_level" required
                                ...
                                <option value="French" {{ old('roast_level') == 'French' ? 'selected' : '' }}>French</option>
                                <option value="Italian" {{ old('roast_level') == 'Italian' ? 'selected' : '' }}>Italian</option>
                            </select>
                            @error('roast_level')
                            <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                            @enderror
                ...
                <div>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="6" required
                                  ...
                                  class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500">{{ old('description') }}</textarea>
                        @error('description')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <div>
                        <label for="image" class="block text-gray-300 text-sm font-medium mb-2">Product Image</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="processing_method" id="processing_method"
                                ...
                                <option value="Anaerobic" {{ old('processing_method') == 'Anaerobic' ? 'selected' : '' }}>Anaerobic</option>
                                <option value="Mixed" {{ old('processing_method') == 'Mixed' ? 'selected' : '' }}>Mixed</option>
                            </select>
                        </div>
                        
                        <div>
                            <label for="altitude" class="block text-gray-300 text-sm font-medium mb-2">Altitude (masl)</label>
                            <input type="text" name="altitude" id="altitude" value="{{ old('altitude') }}"
